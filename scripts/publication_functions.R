...
#
#   outputs:  
publication_ordination <- function(data_list,
                           data_names,
                           metadata,
                           sample_var,
                           hull_var,
    ...
    for( l in 1:length(local_obj) ) {
        ...
        if( method == 'NMDS' ) {
            ...
            ord.res <- metaMDS(t_data, autotransform=F, parallel=7, trymax=200)
        ...
        temp_ord_groups <- as.character(ord_points$Group_Var)
        temp_ord_groups <- gsub('Clitoris', 'Clitoral Fossa', temp_ord_groups)
        temp_ord_groups <- gsub('Vagina', 'Vaginal Vault', temp_ord_groups)
        temp_ord_groups <- gsub('Vestibule', 'Vaginal Vestibule', temp_ord_groups)
        
        ord_points[['Group_Var']] <- factor(temp_ord_groups, levels=c('Clitoral Fossa', 'Vaginal Vestibule', 'Vaginal Vault', 'Uterus'),
                                       ordered=T)
        
        temp_hull_groups <- as.character(hulls$Group_Var)
        temp_hull_groups <- gsub('Clitoris', 'Clitoral Fossa', temp_hull_groups)
        temp_hull_groups <- gsub('Vagina', 'Vaginal Vault', temp_hull_groups)
        temp_hull_groups <- gsub('Vestibule', 'Vaginal Vestibule', temp_hull_groups)
        
        hulls[['Group_Var']] <- factor(temp_hull_groups, levels=c('Clitoral Fossa', 'Vaginal Vestibule', 'Vaginal Vault', 'Uterus'),
                                  ordered=T)
...
publication_alpha_rarefaction <- function(data_list,
                                  data_names,
                                  metadata,
                                  sample_var,
                                  group_var,
    ...
    for( l in 1:length(local_data) ) {
        ...
        local_obj <- alpha_rarefaction(MRcounts(local_data[[l]]), minlevel = 100)
    ...
    alphadiv_type_sums <- all_alphadiv[Level=='Phylum', median(Value), by=group_var]
    alphadiv_value_labels <- as.character(alphadiv_type_sums[[group_var]][order(alphadiv_type_sums$V1, decreasing=T)])
    
    species_raw_type_sums <- all_species_raw[Level=='Phylum', median(Value), by=group_var]
    species_raw_value_labels <- as.character(species_raw_type_sums[[group_var]][order(species_raw_type_sums$V1,
                                                                                decreasing=T)])
    species_rare_type_sums <- all_species_rare[Level=='Phylum', median(Value), by=group_var]
    species_rare_value_labels <- as.character(species_rare_type_sums[[group_var]][order(species_rare_type_sums$V1,
                                                                                       decreasing=T)])
    
    species_raw_value_labels <- gsub('Clitoris', 'Clitoral Fossa', species_raw_value_labels)
    species_raw_value_labels <- gsub('Vagina', 'Vaginal Vault', species_raw_value_labels)
    species_raw_value_labels <- gsub('Vestibule', 'Vaginal Vestibule', species_raw_value_labels)
    
    
    temp_all_alphadiv_labels <- as.character(all_alphadiv[[group_var]])
    temp_all_alphadiv_labels <- gsub('Clitoris', 'Clitoral Fossa', temp_all_alphadiv_labels)
    temp_all_alphadiv_labels <- gsub('Vagina', 'Vaginal Vault', temp_all_alphadiv_labels)
    temp_all_alphadiv_labels <- gsub('Vestibule', 'Vaginal Vestibule', temp_all_alphadiv_labels)
    
    
    temp_all_species_raw_labels <- as.character(all_species_raw[[group_var]])
    temp_all_species_raw_labels <- gsub('Clitoris', 'Clitoral Fossa', temp_all_species_raw_labels)
    temp_all_species_raw_labels <- gsub('Vagina', 'Vaginal Vault', temp_all_species_raw_labels)
    temp_all_species_raw_labels <- gsub('Vestibule', 'Vaginal Vestibule', temp_all_species_raw_labels)
    
    
    test1 <<- all_alphadiv
    
    all_alphadiv[[group_var]] <- factor(temp_all_alphadiv_labels,
                                     levels=species_raw_value_labels, ordered=T)
    
    all_species_raw[[group_var]] <- factor(temp_all_species_raw_labels,
                              levels=species_raw_value_labels, ordered=T)
    ...
    all_alphadiv[, Measure := ( rep('Alpha Diversity', nrow(all_alphadiv)) )]
    all_species_raw[, Measure := ( rep('Species Richness', nrow(all_species_raw)) )]
    
    all_alphadiv <- rbind(all_alphadiv, all_species_raw)
    ...
    g_alphadiv <- g_alphadiv +
        ylab('Inverse Simpson\'s Index or Count\n') +
        xlab('\nSampling Location') +
        theme(panel.grid.major = element_blank(),
              panel.grid.minor = element_blank(),
